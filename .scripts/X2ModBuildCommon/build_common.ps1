...
class BuildProject {
	...
	BuildProject(
		...
		$this.sdkPath = $sdkPath + "\\XCOM 2 War of the Chosen SDK"
		$this.gamePath = $gamePath + "\\XCom2-WarOfTheChosen"
	...
	[void]InvokeBuild() {
		try {
			...
			$this._PerformStep({ ($_)._FinalCopy() }, "Copying", "Copied", "built mod to game and target directories")
	...
	[void]_ConfirmPaths() {
		...
		Write-Host "Target Path: $($this.targetPath)"
	
		# Check if the user config is set up correctly
		if (([string]::IsNullOrEmpty($this.sdkPath) -or $this.sdkPath -eq '${config:xcom.highlander.sdkroot}') -or ([string]::IsNullOrEmpty($this.gamePath) -or $this.gamePath -eq '${config:xcom.highlander.gameroot}') -or ([string]::IsNullOrEmpty($this.targetPath) -or $this.targetPath -eq '${config:xcom.highlander.targetpath}'))
		{
			ThrowFailure "Please set up user config xcom.highlander.sdkroot, xcom.highlander.gameroot and xcom.highlander.targetpath"
		}
		...
		elseif (!(Test-Path $this.targetPath))
		{
			ThrowFailure ("The path '{}' doesn't exist. Please adjust the xcom.highlander.targetpath variable in your user config and retry." -f $this.targetPath)
		}
	}
	...
	[void]_SetupUtils() {
		...
		$this.finalTargetPath = "$($this.targetPath)\$($this.modNameFull)"
	...
	[void]_FinalCopy() {
		...
		Robocopy.exe "$($this.stagingPath)" "$($this.finalTargetPath)" *.* $global:def_robocopy_args
	}
